...
function Event(props)
{
    ...
    const event = useSelector(({eventStore}) => eventStore.event);
    ...
    useEffect(() => {
        function updateEventState()
        {
            ...
            if ( eventId === 'new' )
            {
                dispatch(Actions.newEvent());
            }
            else
            {
                dispatch(Actions.getEvent(props.match.params));
            }
        }
        updateEventState();
    ...
    useEffect(() => {
        if (
            (event.data && !form) ||
            (event.data && form && event.data.id !== form.id)
        )
        {
            setForm(event.data);
        }
    ...
    function canBeSubmitted()
    {
        return (
            form.name.length > 0 &&
            !_.isEqual(event.data, form) && form.images.length > 0
        );
    }
        ...
        <FusePageCarded
                    ...
                    <div className="flex flex-1 w-full items-center justify-between">
                        ...
                        <div className="flex flex-col items-start max-w-full">
                            ...
                            <FuseAnimate animation="transition.slideRightIn" delay={300}>
                                <Typography className="normal-case flex items-center sm:mb-12" component={Link} role="button" to="/events" color="inherit">
            ...
            contentToolbar={
                ...
                >
                    ...
                    <Tab className="h-64 normal-case" label="Event Images"/>
                                ...
                                <div className="flex justify-center sm:justify-start flex-wrap">
                                    ...
                                    {!!form.images && form.images.length > 0 && form.images.map(media => (
                                        <div
                                            ...
                                            className={
                                                clsx(
                                                    classes.eventImageItem,
                                                    "flex items-center justify-center relative w-128 h-128 rounded-4 mr-16 mb-16 overflow-hidden cursor-pointer shadow-1 hover:shadow-5",
                                                    (media.id === form.featuredImageId) && 'featured')
                                            }
                                            key={media.id}
                                        >
                                            <Icon className={classes.eventImageFeaturedStar}>star</Icon>
                                            <img className="max-w-none w-auto h-full" src={media.url} alt="event"/>
                                        </div>
                                    ))}
                                </div>
                            </div>
...
export default withReducer('eventStore', reducer)(Event);
