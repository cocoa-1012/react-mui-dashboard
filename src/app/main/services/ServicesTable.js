...
function ServicesTable(props)
{
    ...
    const services = useSelector(({serviceStore}) => serviceStore.services.data);
    const searchText = useSelector(({serviceStore}) => serviceStore.services.searchText);
    ...
    useEffect(() => {
        setData(searchText.length === 0 ? services : _.filter(services, item => item.name.toLowerCase().includes(searchText.toLowerCase())))
    }, [services, searchText]);
    ...
    {
        props.history.push('/services/' + item.id + '/' + item.handle);
    }
    ...
    function removedSelected() {
        dispatch(Actions.deleteServices(selected));
    }
