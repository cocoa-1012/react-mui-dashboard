...
function PackagesTable(props)
{
    ...
    const packages = useSelector(({packageStore}) => packageStore.packages.data);
    ...
    useEffect(() => {
        setData(searchText.length === 0 ? packages : _.filter(packages, item => item.name.toLowerCase().includes(searchText.toLowerCase())))
    }, [packages, searchText]);
    ...
    {
        props.history.push('/packages/' + item.id + '/' + item.handle);
    }
    ...
    function removedSelected() {
        dispatch(Actions.deletePackages(selected));
    }
